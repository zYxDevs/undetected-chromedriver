...
def main():
    ...
    for item in tmp.rglob('**'):
        ...
        if item.is_dir():
            if 'chrome-' in item.name:
                ...
                logger.info(f'adding {str(item)} to PATH')
                logger.info(f"current PATH: {str(os.environ.get('PATH'))}")
                path_list = os.environ['PATH'].split(os.pathsep)
                path_list.insert(0, str(item))
                os.environ['PATH'] = os.pathsep.join(path_list)
                logger.info(f"new PATH {str(os.environ.get('PATH'))}:")
    ...
    if pdfdata := driver.execute_cdp_cmd('Page.printToPDF', {}):
        if 'data' in pdfdata:
            data = pdfdata['data']
            import base64
